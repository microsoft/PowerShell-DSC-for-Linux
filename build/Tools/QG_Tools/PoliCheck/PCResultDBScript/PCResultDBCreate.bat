...
REM
REM       1) Server Name - Server on which the PoliCheck user wants the PoliCheck Results to be saved 
...
REM	02/16/2006	v-Swetas	Creation of the CMD file to automate the process
REM=======================================================================
...
if "%1" == "" echo Usage - PCResultsDatabase ^<dbservername^> ^<dbname^> & GOTO END
if "%2" == "" echo Usage - PCResultsDatabase ^<dbservername^> ^<dbname^> & GOTO END

REM Log filename variables...
SET log=ResultsDatabase.log
...
REM =========================================================================
		...
		ECHO **** Creating PCResults Database >> %log%
		ECHO **** Creating PCResults Database 
...
osql -E -S %Server% -d %DBName% -i "PCResultDBScript.sql" -n >> %log%
