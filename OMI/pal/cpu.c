    ...
    _Out_ PAL_Datetime* current)
{
    ...
    // this is probably not a big problem for actual time, but I pointed out to Ryan that range of SystemTime.wYear (1601-30827) 
    // is not a subset of MI_Timestamp.year (0-9999 according to DMTF + verified by asserts in Mint + assumed in various integer 
    // overflow considerations for arithmetic/comparison built-ins)
