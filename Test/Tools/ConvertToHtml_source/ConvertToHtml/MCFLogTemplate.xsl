...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
      ...
      <xsl:choose>
        <xsl:when test="name() = 'Run'">
          ...
          <tr>
            ...
            <th extra="true">Error Message</th>
          </tr>
        </xsl:when>
  ...
  <xsl:template name="ErrorMessage">
    <xsl:param name="text"></xsl:param>
    <xsl:variable name="cr" select="'&#xa;'"/>
    <xsl:variable name="after" select="'Infra.Frmwrk.VarFail:'"/>
    <xsl:variable name="before" select="'at Scx'"/>
    <xsl:choose>
      <xsl:when test="contains($text,$cr)">
        <xsl:value-of select="substring-before($text,$before)"/>
        <br />
        <xsl:call-template name="ErrorMessage">
          <xsl:with-param name="text" select="substring($text,$after,$before)"/>
        </xsl:call-template>
      </xsl:when>
    </xsl:choose>
  </xsl:template>
