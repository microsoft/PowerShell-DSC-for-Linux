...
int Vsnprintf_CultureInvariant(
    ...
    va_list ap)
{
    /* TODO/FIXME - ideally we would avoid calling setlocale and use _vsprintf_s_l
...
int Vswprintf_CultureInvariant(
    ...
    va_list ap)
{
    /* TODO/FIXME - ideally we would avoid calling setlocale and use _vswprintf_s_l
    instead.  The problem is that _create_locale is not exported on Win7.  */
    ...
    va_list ap)
{
    ...
    {
        /* TODO/FIXME - ideally we would avoid calling setlocale and use _sscanf_l
           instead.  The problem is that _create_locale is not exported on Win7 */
        char oldLocale[128];
        Strlcpy(oldLocale, setlocale(LC_ALL, NULL), PAL_COUNT(oldLocale));
        setlocale(LC_ALL, "C");
