...
if nxNPMD != None:
    class nxOMSAgentNPMConfigTestCases(unittest2.TestCase):
        ...
        def setUp(self):
            ...
            self.contents = base64.b64encode(b'<Configuration></Configuration>').decode('ascii', 'ignore')
            self.content_checksum = hashlib.md5(self.contents.encode('utf-8')).hexdigest().upper()
        ...
        def setupNPMWithConfigFile(self, workspaceId):
            ...
            file_contents = base64.b64decode(self.contents.encode('utf-8')).decode('ascii', 'ignore')
        ...
        def testTestConfigUpdateCases(self):
            # generate test data
            test_content01 = base64.b64encode(b'<Configuration>Test1</Configuration>').decode('ascii', 'ignore')
            test_content01_checksum = hashlib.md5(test_content01.encode('utf-8')).hexdigest().upper()
            test_content02 = base64.b64encode(b'<Configuration>Test2</Configuration>').decode('ascii', 'ignore')
            test_content02_checksum = hashlib.md5(test_content02.encode('utf-8')).hexdigest().upper()
            test_content03 = base64.b64encode(b'<Configuration>Test3</Configuration>').decode('ascii', 'ignore')
            test_content03_checksum = hashlib.md5(test_content03.encode('utf-8')).hexdigest().upper()
        ...
        def testSetChecksumMismatchCase(self):
            test_content = base64.b64encode(b'SomeRandomValue').decode('ascii', 'ignore')
        ...
        def testSetSolutionEnableCases(self):
            ...
            conf_file_checksum = hashlib.md5(base64.b64encode(self.readFile(conf_file_path).encode('utf-8'))).hexdigest().upper()
        ...
        def testSetAgentConfigUpdateCases(self):
            ...
            self.setupNPMWithConfigFile(self.config_id)
            # generate test data
            test_content01_str = b'<Configuration>Test1</Configuration>'
            test_content01 = base64.b64encode(test_content01_str).decode('ascii', 'ignore')
            test_content01_checksum = hashlib.md5(test_content01.encode('utf-8')).hexdigest().upper()
            test_content02_str = b'<Configuration>Test2</Configuration>'
            test_content02 = base64.b64encode(test_content02_str).decode('ascii', 'ignore')
            test_content02_checksum = hashlib.md5(test_content02.encode('utf-8')).hexdigest().upper()
            test_content03_str = b'<Configuration>Test3</Configuration>'
            test_content03 = base64.b64encode(test_content03_str).decode('ascii', 'ignore')
            test_content03_checksum = hashlib.md5(test_content03.encode('utf-8')).hexdigest().upper()
            # create file paths
            ...
            self.assertTrue(str(test_content01_str, 'utf8') == read_data, 'Valid agent config update should happen for ws01 as it is running NPM')
            ...
            self.assertTrue(str(test_content02_str, 'utf8') == read_data, 'Valid agent config update should happen for ws02 as it used ensure_present even when not running NPM')
            ...
            # Verify that update has occurred even for ws02 as ensure == present
            read_data = self.readFile(agent_conf_file_ws02)
            self.assertTrue(str(test_content03_str, 'utf8') != read_data, 'Valid agent config update should not happen for ws02 as it used ensure_absent')
