...
def init_locals(WorkspaceId, AzureDnsAgentSvcZone):
    ...
    return str(WorkspaceId).encode('ascii', 'ignore'), str(AzureDnsAgentSvcZone).encode('ascii', 'ignore')
...
def set_marshall_helper(WorkspaceId, Enabled, AzureDnsAgentSvcZone, mock_worker_config_file=False):
    try:
        is_primary = is_workspace_primary(WorkspaceId)
    ...
    if Enabled:
        try:
            ...
            if proc.wait() != 0:
                log(ERROR, "Linux Hybrid Worker registration failed: " + str(stderr) + "\n" + str(stdout))
                return [-1]
...
def Test_Marshall(WorkspaceId, Enabled, AzureDnsAgentSvcZone):
    ...
    try:
        isprimary = is_workspace_primary(WorkspaceId)
    except Exception as e:
        log(ERROR, "Could not detect if workspace is primary\n %s" %(str(e)))
        return [-1]
...
def is_workspace_primary(mof_workspace_id):
    """
...
def kill_hybrid_worker():
    ...
    if pid > 0:
        try:
            ...
            log(ERROR, "Could not kill Linux Hybrid Worker process pid: " + str(pid) + " " + str(exception))
            raise exception
