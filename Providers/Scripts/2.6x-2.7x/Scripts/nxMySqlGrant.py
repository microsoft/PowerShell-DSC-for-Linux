...
def IsPermissionPresent(UserName, DatabaseName, ConnectionCredential, PermissionType):
    cmd='mysql -u root -e "SHOW GRANTS FOR '  + UserName+"@'%';"+'"'
    os.environ['MYSQL_PWD'] = ConnectionCredential
    code, out=RunGetOutput(cmd,False)
    os.environ['MYSQL_PWD'] = ''
    perm="GRANT " + PermissionType + " ON `" + DatabaseName + "`.* TO '" + UserName + "'"
    if perm in out:
        return True
    return False
...
def AddRemovePermission(UserName, DatabaseName,  ConnectionCredential, PermissionType, Ensure):
    ...
    if Ensure == 'Present':
        if present:
            ...
            cmd="GRANT " + PermissionType + " ON " + DatabaseName + ".* TO " + UserName + ";"
    elif present:
        cmd = "REVOKE "+ PermissionType + " ON " + DatabaseName + ".* FROM " + UserName + ";"
    if len(cmd) > 0 :
