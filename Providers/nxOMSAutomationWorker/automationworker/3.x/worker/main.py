...
class WorkerManager(object):
    ...
    def get_worker_and_sandbox_processes(process_list):
        ...
        for process in process_list:
            if ("python" in process.cmd or "python2" in process.cmd or "python3" in process.cmd) and ("hybridworker.py" in process.cmd or "sandbox.py" in process.cmd) \
    ...
    def get_worker_processes(process_list):
        ...
        worker_processes = []
        for process in process_list:
            if ("python" in process.cmd or "python2" in process.cmd or "python3" in process.cmd) and "hybridworker.py" in process.cmd \
                    and "MSFT_nxOMSAutomationWorkerResource" in process.cmd:
...
class OMSConfiguration(object):
    ...
    KEY_HYBRID_WORKER_PATH_FORP3 = "hybrid_worker_path_p3"
    KEY_DISABLE_WORKER_CREATION = "disable_worker_creation"
    KEY_WORKSPACE_ID = "workspace_id"

    DEFAULT_EMPTY_VALUE = ""
    DEFAULT_HYBRID_WORKER_PATH = "/opt/microsoft/omsconfig/modules/nxOMSAutomationWorker/DSCResources/MSFT_nxOMSAutomationWorkerResource/automationworker/3.x/worker/hybridworker.py"
    DEFAULT_HYBRID_WORKER_PATH_FORP3 = "/opt/microsoft/omsconfig/modules/nxOMSAutomationWorker/DSCResources/MSFT_nxOMSAutomationWorkerResource/automationworker/3.x/worker/hybridworker.py"
    ...
    def get_config_reader(self):
        config = configparser.SafeConfigParser({self.KEY_AUTO_REGISTERED_WORKER_CONF_PATH: self.DEFAULT_EMPTY_VALUE,
                                                ...
                                                self.KEY_HYBRID_WORKER_PATH_FORP3: self.DEFAULT_HYBRID_WORKER_PATH_FORP3,
    ...
    def get_hybrid_worker_source_path(self):
        config = self.get_config_reader()
        return config.get(self.SECTION_OMS_GLOBAL, self.KEY_HYBRID_WORKER_PATH_FORP3)
