...
from worker import configuration2
...
def create_worker_configuration_file(jrds_uri, automation_account_id, worker_group_name, machine_id,
    ...
    worker_required_section = configuration2.WORKER_REQUIRED_CONFIG_SECTION
    if not config.has_section(worker_required_section):
        config.add_section(worker_required_section)
    config.set(worker_required_section, configuration2.CERT_PATH, cert_path)
    config.set(worker_required_section, configuration2.KEY_PATH, key_path)
    config.set(worker_required_section, configuration2.BASE_URI, jrds_uri)
    config.set(worker_required_section, configuration2.ACCOUNT_ID, automation_account_id)
    config.set(worker_required_section, configuration2.MACHINE_ID, machine_id)
    config.set(worker_required_section, configuration2.HYBRID_WORKER_GROUP_NAME, worker_group_name)
    config.set(worker_required_section, configuration2.WORKING_DIRECTORY_PATH, working_directory_path)

    worker_optional_section = configuration2.WORKER_OPTIONAL_CONFIG_SECTION
    if not config.has_section(worker_optional_section):
        config.add_section(worker_optional_section)
    config.set(worker_optional_section, configuration2.PROXY_CONFIGURATION_PATH,
               "/etc/opt/microsoft/omsagent/proxy.conf")
    config.set(worker_optional_section, configuration2.STATE_DIRECTORY_PATH, state_directory_path)
    if gpg_keyring_path is not None:
        config.set(worker_optional_section, configuration2.GPG_PUBLIC_KEYRING_PATH, gpg_keyring_path)
    if test_mode is True:
        config.set(worker_optional_section, configuration2.BYPASS_CERTIFICATE_VERIFICATION, True)
        config.set(worker_optional_section, configuration2.DEBUG_TRACES, True)

    metadata_section = configuration2.METADATA_CONFIG_SECTION
    if not config.has_section(metadata_section):
        config.add_section(metadata_section)
    config.set(metadata_section, configuration2.WORKER_TYPE, "diy")
    config.set(metadata_section, configuration2.IS_AZURE_VM, str(is_azure_vm))
    config.set(metadata_section, configuration2.VM_ID, vm_id)
    ...
    config.set(registration_metadata_section, configuration2.REGISTRATION_ENDPOINT, registration_endpoint)
    config.set(registration_metadata_section, configuration2.WORKSPACE_ID, workspace_id)
